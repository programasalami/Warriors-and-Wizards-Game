...
# tile art -------------------------------------------------------------------------------------
ground = Image.open(f'{SHEETS}/ForestGround.png').convert('RGBA')
edges = Image.open(f'{SHEETS}/ForestGroundEdge.png').convert('RGBA')
...
# decor -----------------------------------------------------------------------------------------
# (decor-sheet index, base X, base Y in the 1280x720 design canvas, sways?) - base = where the object stands.
...
for idx, bx, by, sway in DECOR:
    w, h = round(CELL_W * PIXEL_SCALE), round(CELL_H * PIXEL_SCALE)
    cs.append(f'        new({idx}, {bx}, {by}, {w}, {h}, {round(INSET[idx] * h)}, {str(sway).lower()}),')
cs += ['    ];', '}', '']
