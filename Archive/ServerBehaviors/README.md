...
These are the original game's enemy AI scripts (`BehaviorLib.<area>.cs` partial classes from the upstream alloy-server): Abyss, Avatar,
...
To bring one back: rename it to `.cs`, move it to `alloy-server/GameServer/Game/Systems/Behaviors/Library/`, give the enemy an
