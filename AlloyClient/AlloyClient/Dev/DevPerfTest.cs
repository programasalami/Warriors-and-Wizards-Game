...
namespace AlloyClient.Dev;

// A scripted, repeatable frame-cost measurement (2026-09-22; lives in Dev/ with nothing else of the game - developer tooling, switched off for players). OFF unless the environment variable ALLOY_PERFTEST names an output file, so a
...
public static class DevPerfTest {
    public static readonly string OutputPath = Environment.GetEnvironmentVariable("ALLOY_PERFTEST");
    ...
    // ALLOY_PERFTEST_MODE=enemies: real server traffic instead of fake players - /god on, then /spawn 10 Pirate next to you (owner commands;
    ...
    public static readonly bool EnemyMode = Environment.GetEnvironmentVariable("ALLOY_PERFTEST_MODE") == "enemies";

    // ALLOY_PERFTEST_MODE=timeline: enter the NEXUS and stand still for up to 15 minutes, appending one line every 5 s (other players in the
    ...
    public static readonly bool TimelineMode = Environment.GetEnvironmentVariable("ALLOY_PERFTEST_MODE") == "timeline";
    ...
    public static void Update(in GameTime time) {
        ...
        tot.UiBatches += Alloy.UiLib.Rendering.SpriteRender.LastBatchCount;
