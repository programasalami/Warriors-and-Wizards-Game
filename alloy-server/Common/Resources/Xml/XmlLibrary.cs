...
namespace Common.Resources.Xml
{
    public static class XmlLibrary
    {
        ...
        private static readonly ConcurrentDictionary<string, Item> _id2Item =
            new(StringComparer.CurrentCultureIgnoreCase);
        ...
        /// <summary>
        /// Loads every .xml file in the directory.
        /// </summary>
        public static void Load(string dir)
        {
            ...
            foreach (var file in files)
            {
                ...
                var root = XElement.Parse(File.ReadAllText(file));
                MakeDictionaries(root);
            }
        ...
        private static void MakeDictionaries(XElement root)
        {
            foreach (var xml in root.Elements())
            {
                ...
                if (name == "Object")
                {
                    ...
                    {
                        ContainerDescs.TryAdd(
                            type,
                            new ContainerDesc(xml, id, type));
                    }
                    ...
                    if (!xml.HasElement("Item") &&
                        !xml.HasElement("PetAbility"))
                    {
                        ...
                        if (objDesc.Terrain != TerrainType.None)
                        {
                            var terrain = Enum.Parse<TerrainType>(
                                xml.Element("Terrain").Value);

                            if (!TerrainEnemies.TryGetValue(
                                    terrain,
                                    out var list))
                            {
                                list = new List<ObjectDesc>();
                                TerrainEnemies.TryAdd(terrain, list);
                            }
        ...
        public static ObjectDesc Id2Object(
            string id,
