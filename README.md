...
| Folder | What it is |
| --- | --- |
| `WaWClient/` | The desktop client (C# / .NET 10, OpenTK, OpenGL 4.3). `WarriorsAndWizards.Client.sln` holds the engine, UI, audio, content builder and the game. |
...
| `WaW-Server/` | `AccountServer` (HTTP :8080, accounts / characters / rewards, Postgres + Redis) and `GameServer` (TCP :2050, the simulation). `WarriorsAndWizards.Server.sln`. |
...
```
dotnet build WaW-Client/WarriorsAndWizards.Client.sln          # the client; build the SOLUTION, not the single project
dotnet test  WaW-Client/WarriorsAndWizards.Client.sln
dotnet build WaW-Server/WarriorsAndWizards.Server.sln     # both servers
dotnet test  WaW-Server/WarriorsAndWizards.Server.sln
...
1. Install PostgreSQL 17 and Redis (Memurai on Windows). Create an empty database and put its settings in
   `WaW-Server/Common/Resources/Config/Data/postgresConfig.xml` (copy the `.example.xml`; the real file is git-ignored). The
   servers create their own tables.
2. Start `WaW-Server/bin/debug/net10.0/AccountServer.exe`, then `GameServer.exe` (each in its own window).
3. Start `WaW-Client/WaWClient/bin/Debug/net10.0/WaWClient.exe`. The source is configured for `127.0.0.1`; `deploy.ps1`
...
Logging: the client logs at Information (`WAW_LOG=debug` for more); the servers skip Debug lines unless `WAW_DEBUG_LOG` is set.
