    ...
    private void AddComponents(ref Entity en) {
        ...
        switch (en.Type) {
            ...
            case EntityType.Player:
                // 0-19 as the class describes them (gear, inventory, backpack), then the Skill slot(s), which take only Skill items.
                var classSlots = XmlLibrary.PlayerDescs[en.Desc.ObjectType].SlotTypes;
                var slotTypes = new int[InventoryLayout.PlayerSlots];
                classSlots.AsSpan(0, Math.Min(classSlots.Length, InventoryLayout.SkillSlot)).CopyTo(slotTypes);
                for (var s = InventoryLayout.SkillSlot; s < InventoryLayout.PlayerSlots; s++)
                    slotTypes[s] = InventoryLayout.SkillSlotType;
                inv = new EntityInventory(this, ref en, InventoryLayout.PlayerSlots);
                inv.Init(slotTypes, []);
                EntityInventories.Add(ref inv);
