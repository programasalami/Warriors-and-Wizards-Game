...
public static class Settings {
    ...
    private const string LocalFolderName = "WaWClient";
    private const string OldLocalFolderName = "AlloyClient";   // the folder's name before the 2026-09-23 rename; moved over once (MoveOldLocalFolder)
    ...
    public const string BuildVersion = "0.3.8";
    public const string BuildLabel = $"WaW v{BuildVersion}";
    ...
    // CameraAngle is the LIVE angle (rotating changes it); DefaultCameraAngle is the player's choice in Options, which the camera starts at
    // on every world entry and returns to on the reset key. Until 2026-09-23 both were CameraAngle, so the last angle you played at replaced
    // the chosen default and the next session started there.
    public static readonly ValueSetting<float> CameraAngle = new(0f);
    public static readonly ValueSetting<float> DefaultCameraAngle = new(0f);
    ...
    static Settings() {
        ...
        var localFolderPath = Path.CombineAlt(localAppData, LocalFolderName);
        MoveOldLocalFolder(Path.CombineAlt(localAppData, OldLocalFolderName), localFolderPath);
    ...
    // Saved settings and sign-in lived in %LocalAppData%\AlloyClient until the 2026-09-23 rename: move that folder to the new name once, so
    // nobody's options or login reset. When the move fails (a file in use), copy the files instead. Never throws: the worst case is defaults.
    private static void MoveOldLocalFolder(string oldPath, string newPath) {
        try {
            ...
            try {
                ...
                foreach (var file in Directory.GetFiles(oldPath))
                    File.Copy(file, Path.CombineAlt(newPath, Path.GetFileName(file)), false);
            }
        } catch (Exception) {
            // unreadable old folder: start with defaults like a new install
        }
    }
