...
public sealed class ItemTile : Sprite {
    ...
    public ItemTile(Entity owner, byte slotId, bool interactive, CutEdges cut, bool oneWay, byte slotType = 0, int tileSize = 50, uint bgcolor = 0x545454) {
        ...
        if (SlotType == ItemConstants.SkillType) {
            _slotId.SetText("S");          // an empty Skill slot shows a greyed S (the pack has no skill picture yet)
        }
        if (ShowsLabel) {
    ...
    public void SetItem(ItemDesc itemDesc) {
        ...
        if (ItemDesc != null && ItemDesc.ObjectType > 0) {
            ...
            _unusable.Visible = false;
            if (ShowsSilhouette) _slotDetail.Visible = true;
            if (ShowsLabel) _slotId.Visible = true;
    ...
    // What an EMPTY slot shows: a gear slot the faint picture of its kind, an inventory slot its number, the Skill slot a greyed "S".
    private bool ShowsSilhouette => SlotType != 0 && SlotType != ItemConstants.SkillType;
    private bool ShowsLabel => (Owner is Player && SlotType == 0) || SlotType == ItemConstants.SkillType;
    ...
    private static bool IsUsableByPlayer(ItemDesc itemDesc) {
        ...
        if (slotType == ItemConstants.PotionType || slotType == ItemConstants.SkillType)
            return true;       // every class may hold a Skill for now (per-class skills can come later through the item's own rules)
    ...
    private void DoubleClick() {
        ...
        if (ItemDesc.SlotType == ItemConstants.SkillType) {
            // a Skill goes to the Skill slot, or from it back to the first free inventory slot
            var skillTarget = InventoryLayout.SkillSlot;
            if (InventoryLayout.IsSkillSlot(SlotId)) {
                skillTarget = -1;
                for (var i = InventoryLayout.GearSlots; i < 12; i++)
                    if (Owner.Equipment[i] == null) { skillTarget = i; break; }
            }
            if (skillTarget >= 0)
                QueueSwap(SlotId, skillTarget);
            return;
        }
        ...
        QueueSwap(SlotId, target);
    }

    private void QueueSwap(byte from, int to) {
        var swap = InvSwap.CreatePacket();
        swap.SlotObj1 = new ObjectSlot { ObjectId = Owner.ObjectId, SlotId = from };
        swap.SlotObj2 = new ObjectSlot { ObjectId = Owner.ObjectId, SlotId = (byte)to };
        Client.QueuePacket(swap);
    }
