...
public sealed class HudView : Sprite {
    ...
    private SkillBar _skills;              // the Skill row above the gear (2026-09-23)
    ...
    public void CreatePlayerDependentAssets() {
        ...
        _skills = new SkillBar(Map.LocalPlayer);
        AddChild(_skills);
    ...
    public void Layout(int designW, int designH) {
        ...
        // the Skill row hugs the left edge right above the gear
        _skills.X = Margin;
        _skills.Y = _gear.Y - SkillBar.Height - GearGap;
        ...
        var bottom = (int) _skills.Y;             // the popups stop above the Skill row
    ...
    private IEnumerable<(float X, float Y, float W, float H)> Rects() {
        ...
        yield return (_skills.X, _skills.Y, SkillBar.Width, SkillBar.Height);
