...
using WaWClient.Game.Components.Options.OptionTypes;
using WaWClient.Ui.Components.Scrollbars;
using WaW.UiLib.BuiltIn;
using WaW.UiLib.Core;
using OpenTK.Platform;

namespace WaWClient.Game.Components.Options;
...
public class OptionTabView : Container {
    ...
    public OptionTabView(string name) : base(new ContainerConfig { Width = OptionsView.ViewWidth, Height = OptionsView.ViewHeight, EnableClip = true }) {
        ...
        switch (name) {
            case OptionsView.GeneralTab:
                AddGeneralOptions();
                break;
            case OptionsView.InventoryTab:
                AddInventoryOptions();
                break;
    ...
    // General (was "Controls" until 2026-09-23), grouped like the Extra tab.
    ...
    // Inventory (was "Hot Keys" until 2026-09-23): the keys that use an inventory slot.
    ...
    private void AddExtraOptions() {
        ...
        _options.Add(new ChoiceOption<float>(Settings.DefaultCameraAngle, ["45", "0"], [7 * MathF.PI / 4, 0f], "Default Camera Angle",
            "The angle the camera starts at when you enter a world, and returns to on the Reset To Default Camera Angle key", OnDefaultCameraAngleChange));
    ...
    private void OnDefaultCameraAngleChange() {
        Settings.CameraAngle.Set(Settings.DefaultCameraAngle);   // show the new default straight away
    }
