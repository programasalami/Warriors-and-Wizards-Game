...
using WaWClient.ParticleEffects;
...
public class Entity {
    ...
    public ItemDesc[] Equipment = new ItemDesc[InventoryLayout.PlayerSlots];   // 0-3 gear, 4-11 inventory, 12-19 backpack, 20 Skill
        ...
        for (var i = 0; i < count; i++) {
            ...
            switch (stat.Type) {
                ...
                case StatsType.BackPack7:
                    ...
                    InventoryUpdate.Dispatch(index);
                    break;
                case StatsType.Skill0:
                    index = InventoryLayout.SkillSlot + stat.Type - StatsType.Skill0;
                    if (stat.Value == -1) {
                        Equipment[index] = null;
                    } else if (Equipment[index] == null || stat.Value != Equipment[index].ObjectType) {
                        Equipment[index] = ObjectLibrary.TypeToItem[(ushort)stat.Value];
                    }
                    InventoryUpdate.Dispatch(index);
                    break;
