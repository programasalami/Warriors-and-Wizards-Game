...
public sealed class TypeWall : RenderBase, IStaticProp {
    ...
    // The wall and its top, baked into a static area mesh (StaticProps). Both sort by the wall's tile centre.
    public void Bake(List<ModelVertexExpanded> into) {
        var code = Render.BakedDepthCode(Render.BakedAtTileCentre, Entity.Jitter);
        Render.BakeModel(ModelType, new VertexModel(Position, UV, new Vector3(0, code, RenderConfig.Shade)), into);
        Top.BakeInto(code, into);
    }
}