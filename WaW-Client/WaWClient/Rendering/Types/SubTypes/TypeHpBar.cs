...
public class TypeHpBar : SubRenderBase {
    ...
    public void SetFill(float percent) {
        ...
        // The quad is centred on its offset (corners +-0.5 * width), so keeping the LEFT end still means moving the centre by HALF the
        // width lost. It used to move by the whole width, so the fill slid left off its background as health dropped (2026-09-23).
        Scale.X = 0.68f * percent;
        Scale.Z = (0.68f * percent - 0.68f) / 2f;
    }
