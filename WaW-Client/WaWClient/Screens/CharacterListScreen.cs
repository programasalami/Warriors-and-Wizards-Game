...
public class CharacterListScreen : TitleScreenBase {
    ...
    // WAW_PERFTEST only (Game/DevPerfTest.cs): play the last-played character by itself.
    ...
    public override void Update(WaW.Engine.GameTime gameTime) {
        ...
        if (!WaWClient.Dev.DevPerfTest.Enabled || _perfTestPressed)
            return;
        ...
        if (WaWClient.Dev.DevPerfTest.TimelineMode)
