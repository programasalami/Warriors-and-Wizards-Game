...
using WaWClient.Screens.Components.Containers;
using WaWClient.Ui.Components.Buttons;
using WaWClient.Ui.Components.Dialogs;
using WaWClient.Ui.Components.Graphics;
...
public class TitleScreen : TitleScreenBase {
    ...
    // WAW_PERFTEST only (Game/DevPerfTest.cs): press PLAY by itself once the saved sign-in is loaded.
    private double _perfTestWaitMs;
    private bool _perfTestPressed;

    public override void Update(WaW.Engine.GameTime gameTime) {
        base.Update(gameTime);
        if (!WaWClient.Dev.DevPerfTest.Enabled || _perfTestPressed || !GlobalData.Contains<LoginData>())
            return;
